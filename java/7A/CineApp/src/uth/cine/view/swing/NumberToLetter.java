...
public class NumberToLetter {
    ...
    public static String convert(double number) {
        Double myDouble = new Double(number);
            ...
            Integer myInt = myDouble.intValue();

            double decimalPart = number - myInt;

            decimalPart =  Math.rint(decimalPart * 100) / 100;
            ...
            if (decimalPart > 0) {
                ...
                if (decimalString.length() == 3) {
                    decimalString = decimalString.substring(2, 3) + "0";
                } else if (decimalString.length() >= 4) {
                    decimalString = decimalString.substring(2, 4);
                }
            } else {
                decimalString = "00";
            }

            n2t convertidor = new n2t(myInt);

            String cadenaEntero = convertidor.convertirLetras(myInt);

            // Importe con letra
            String importeLetra = "";
            return importeLetra = (cadenaEntero + " pesos " + decimalString + "/100 " + "M.N.").toUpperCase();
    }
